...
OR=read.table("ES_EggStrains", header=TRUE, sep="\t")

GISAID=read.table("GISAID_32278_H3N2_HA1_ES_byJul2016", header=TRUE, sep="\t")
...
pdf("Correlation_AdaptiveDistance_VE.pdf")
...
veE=paste('Vaccine efficacy = ',veT1,sep='')
text(x=disT+1.25,y=veT+0.015,labels=veE, cex= .6, offset = 10)
disT1=formatC(disT,digits=2, format="f") 
disE=paste('Adaptive distance = ',disT1,sep='')
text(x=disT+1.13,y=veT+0.03,labels=disE, cex= .6, offset = 10)
nx=c(0,disT)
ny=c(veT,veT)
...
############### outfile ####################
cat(veE,file="outfile.txt",sep="\n")
cat(disE,file="outfile.txt",append=TRUE)
